...
class BookBackendSync(BookBackend):
    """
    :Constructors:
    
    ::
    
        BookBackendSync(**properties)
    """
    ...
    __dict__ = None # (!) real value is "mappingproxy({'__info__': ObjectInfo(BookBackendSync), '__module__': 'gi.repository.EDataBook', '__gtype__': <GType EBookBackendSync (94654337863280)>, '__doc__': None, '__gsignals__': {}, 'create_contacts': gi.FunctionInfo(create_contacts), 'get_contact': gi.FunctionInfo(get_contact), 'get_contact_list': gi.FunctionInfo(get_contact_list), 'get_contact_list_uids': gi.FunctionInfo(get_contact_list_uids), 'modify_contacts': gi.FunctionInfo(modify_contacts), 'open': gi.FunctionInfo(open), 'refresh': gi.FunctionInfo(refresh), 'remove_contacts': gi.FunctionInfo(remove_contacts), 'do_open_sync': gi.VFuncInfo(open_sync), 'do_refresh_sync': gi.VFuncInfo(refresh_sync), 'parent': <property object at 0x7f09d41ad810>, 'priv': <property object at 0x7f09d41ad950>})"
    __gdoc__ = "Object EBookBackendSync\n\nSignals from EBookBackend:\n  closed (gchararray)\n  shutdown ()\n\nProperties from EBookBackend:\n  cache-dir -> gchararray: Cache Dir\n    The backend's cache directory\n  proxy-resolver -> GProxyResolver: Proxy Resolver\n    The proxy resolver for this backend\n  registry -> ESourceRegistry: Registry\n    Data source registry\n  writable -> gboolean: Writable\n    Whether the backend will accept changes\n\nProperties from EBackend:\n  connectable -> GSocketConnectable: Connectable\n    Socket endpoint of a network service\n  main-context -> GMainContext: Main Context\n    The main loop context on which to attach event sources\n  online -> gboolean: Online\n    Whether the backend is online\n  source -> ESource: Source\n    The data source being acted upon\n  user-prompter -> EUserPrompter: User Prompter\n    User prompter instance\n\nSignals from GObject:\n  notify (GParam)\n\n"
    __gsignals__ = {}
    __gtype__ = None # (!) real value is '<GType EBookBackendSync (94654337863280)>'
    __info__ = ObjectInfo(BookBackendSync)
