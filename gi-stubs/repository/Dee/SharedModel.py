...
class SharedModel(ProxyModel):
    ...
    ::
        ...
        new(name:str) -> Dee.SharedModel
        new_for_peer(peer:Dee.Peer) -> Dee.SharedModel
        new_with_back_end(name:str, back_end:Dee.Model) -> Dee.SharedModel
    """
    ...
    flush_revision_queue = gi.FunctionInfo(flush_revision_queue)
    flush_revision_queue_sync = gi.FunctionInfo(flush_revision_queue_sync)
    ...
    get_flush_mode = gi.FunctionInfo(get_flush_mode)
    ...
    get_peer = gi.FunctionInfo(get_peer)
    ...
    is_leader = gi.FunctionInfo(is_leader)
    is_synchronized = gi.FunctionInfo(is_synchronized)
    new = gi.FunctionInfo(new)
    newv = gi.FunctionInfo(newv)
    new_for_peer = gi.FunctionInfo(new_for_peer)
    new_with_back_end = gi.FunctionInfo(new_with_back_end)
    ...
    props = None # (!) real value is '<gi._gi.GProps object at 0x7f5a2ce70da0>'
    ...
    set_flush_mode = gi.FunctionInfo(set_flush_mode)
    ...
    __dict__ = None # (!) real value is "mappingproxy({'__info__': ObjectInfo(SharedModel), '__module__': 'gi.repository.Dee', '__gtype__': <GType DeeSharedModel (19716928)>, '__doc__': None, '__gsignals__': {}, 'new': gi.FunctionInfo(new), 'new_for_peer': gi.FunctionInfo(new_for_peer), 'new_with_back_end': gi.FunctionInfo(new_with_back_end), 'flush_revision_queue': gi.FunctionInfo(flush_revision_queue), 'flush_revision_queue_sync': gi.FunctionInfo(flush_revision_queue_sync), 'get_flush_mode': gi.FunctionInfo(get_flush_mode), 'get_peer': gi.FunctionInfo(get_peer), 'get_swarm_name': gi.FunctionInfo(get_swarm_name), 'is_leader': gi.FunctionInfo(is_leader), 'is_synchronized': gi.FunctionInfo(is_synchronized), 'set_flush_mode': gi.FunctionInfo(set_flush_mode), 'parent': <property object at 0x7f5a2cf51e58>, 'priv': <property object at 0x7f5a2cf51ea8>})"
    __gdoc__ = 'Object DeeSharedModel\n\nSignals from DeeSharedModel:\n  begin-transaction (guint64, guint64)\n  end-transaction (guint64, guint64)\n\nProperties from DeeSharedModel:\n  peer -> DeePeer: Peer\n    The peer object that monitors the swarm\n  synchronized -> gboolean: Synchronized\n    Whether the model is synchronized with its peers\n  access-mode -> DeeSharedModelAccessMode: Access Mode\n    Access mode used by this shared model\n  flush-mode -> DeeSharedModelFlushMode: Flush mode\n    Determines whether flushes occur automatically\n\nSignals from DeeModel:\n  row-added (DeeModelIter)\n  row-removed (DeeModelIter)\n  row-changed (DeeModelIter)\n  changeset-started ()\n  changeset-finished ()\n\nProperties from DeeProxyModel:\n  back-end -> DeeModel: Back end\n    Back end model\n  proxy-signals -> gboolean: Proxy signals\n    Whether or not to automatically forward signals from the back end\n  inherit-seqnums -> gboolean: Inherit seqnums\n    Whether or not to inherit seqnums\n\nSignals from DeeModel:\n  row-added (DeeModelIter)\n  row-removed (DeeModelIter)\n  row-changed (DeeModelIter)\n  changeset-started ()\n  changeset-finished ()\n\nSignals from DeeModel:\n  row-added (DeeModelIter)\n  row-removed (DeeModelIter)\n  row-changed (DeeModelIter)\n  changeset-started ()\n  changeset-finished ()\n\nSignals from GObject:\n  notify (GParam)\n\n'
    __gsignals__ = {}
    __gtype__ = None # (!) real value is '<GType DeeSharedModel (19716928)>'
    __info__ = ObjectInfo(SharedModel)
