...
class Session(__gi_overrides_GObject.Object):
    ...
    props = None # (!) real value is '<gi._gi.GProps object at 0x7f8e47c742e0>'
    __class__ = None # (!) real value is "<class 'gi.types.GObjectMeta'>"
    __dict__ = None # (!) real value is "mappingproxy({'__info__': ObjectInfo(Session), '__module__': 'gi.repository.Soup', '__gtype__': <GType SoupSession (94750594747712)>, '__doc__': None, '__gsignals__': {}, 'new': gi.FunctionInfo(new), 'abort': gi.FunctionInfo(abort), 'add_feature': gi.FunctionInfo(add_feature), 'add_feature_by_type': gi.FunctionInfo(add_feature_by_type), 'cancel_message': gi.FunctionInfo(cancel_message), 'connect_async': gi.FunctionInfo(connect_async), 'connect_finish': gi.FunctionInfo(connect_finish), 'get_async_context': gi.FunctionInfo(get_async_context), 'get_feature': gi.FunctionInfo(get_feature), 'get_feature_for_message': gi.FunctionInfo(get_feature_for_message), 'get_features': gi.FunctionInfo(get_features), 'has_feature': gi.FunctionInfo(has_feature), 'pause_message': gi.FunctionInfo(pause_message), 'prefetch_dns': gi.FunctionInfo(prefetch_dns), 'prepare_for_uri': gi.FunctionInfo(prepare_for_uri), 'queue_message': gi.FunctionInfo(queue_message), 'redirect_message': gi.FunctionInfo(redirect_message), 'remove_feature': gi.FunctionInfo(remove_feature), 'remove_feature_by_type': gi.FunctionInfo(remove_feature_by_type), 'request': gi.FunctionInfo(request), 'request_http': gi.FunctionInfo(request_http), 'request_http_uri': gi.FunctionInfo(request_http_uri), 'request_uri': gi.FunctionInfo(request_uri), 'requeue_message': gi.FunctionInfo(requeue_message), 'send': gi.FunctionInfo(send), 'send_async': gi.FunctionInfo(send_async), 'send_finish': gi.FunctionInfo(send_finish), 'send_message': gi.FunctionInfo(send_message), 'steal_connection': gi.FunctionInfo(steal_connection), 'unpause_message': gi.FunctionInfo(unpause_message), 'websocket_connect_async': gi.FunctionInfo(websocket_connect_async), 'websocket_connect_finish': gi.FunctionInfo(websocket_connect_finish), 'would_redirect': gi.FunctionInfo(would_redirect), 'do_auth_required': gi.VFuncInfo(auth_required), 'do_authenticate': gi.VFuncInfo(authenticate), 'do_cancel_message': gi.VFuncInfo(cancel_message), 'do_flush_queue': gi.VFuncInfo(flush_queue), 'do_kick': gi.VFuncInfo(kick), 'do_queue_message': gi.VFuncInfo(queue_message), 'do_request_started': gi.VFuncInfo(request_started), 'do_requeue_message': gi.VFuncInfo(requeue_message), 'do_send_message': gi.VFuncInfo(send_message), 'parent': <property object at 0x7f8e47ef96d0>})"
    __gdoc__ = "Object SoupSession\n\nSignals from SoupSession:\n  authenticate (SoupMessage, SoupAuth, gboolean)\n  request-started (SoupMessage, SoupSocket)\n  request-queued (SoupMessage)\n  request-unqueued (SoupMessage)\n  connection-created (GObject)\n  tunneling (GObject)\n\nProperties from SoupSession:\n  proxy-uri -> SoupURI: Proxy URI\n    The HTTP Proxy to use for this session\n  proxy-resolver -> GProxyResolver: Proxy Resolver\n    The GProxyResolver to use for this session\n  max-conns -> gint: Max Connection Count\n    The maximum number of connections that the session can open at once\n  max-conns-per-host -> gint: Max Per-Host Connection Count\n    The maximum number of connections that the session can open at once to a given host\n  use-ntlm -> gboolean: Use NTLM\n    Whether or not to use NTLM authentication\n  ssl-ca-file -> gchararray: SSL CA file\n    File containing SSL CA certificates\n  ssl-use-system-ca-file -> gboolean: Use system CA file\n    Use the system certificate database\n  tls-database -> GTlsDatabase: TLS Database\n    TLS database to use\n  ssl-strict -> gboolean: Strictly validate SSL certificates\n    Whether certificate errors should be considered a connection error\n  async-context -> gpointer: Async GMainContext\n    The GMainContext to dispatch async I/O in\n  use-thread-context -> gboolean: Use thread-default GMainContext\n    Whether to use thread-default main contexts\n  timeout -> guint: Timeout value\n    Value in seconds to timeout a blocking I/O\n  user-agent -> gchararray: User-Agent string\n    User-Agent string\n  accept-language -> gchararray: Accept-Language string\n    Accept-Language string\n  accept-language-auto -> gboolean: Accept-Language automatic mode\n    Accept-Language automatic mode\n  idle-timeout -> guint: Idle Timeout\n    Connection lifetime when idle\n  add-feature -> SoupSessionFeature: Add Feature\n    Add a feature object to the session\n  add-feature-by-type -> GType: Add Feature By Type\n    Add a feature object of the given type to the session\n  remove-feature-by-type -> GType: Remove Feature By Type\n    Remove features of the given type from the session\n  http-aliases -> GStrv: http aliases\n    URI schemes that are considered aliases for 'http'\n  https-aliases -> GStrv: https aliases\n    URI schemes that are considered aliases for 'https'\n  local-address -> SoupAddress: Local address\n    Address of local end of socket\n  tls-interaction -> GTlsInteraction: TLS Interaction\n    TLS interaction to use\n\nSignals from GObject:\n  notify (GParam)\n\n"
    __gsignals__ = {}
    __gtype__ = None # (!) real value is '<GType SoupSession (94750594747712)>'
